...
- **分类管理**: 按功能分类，便于查找和使用
- **Prompt AI 优化**: 模板编辑面板内置 “AI 优化” 按钮，可调用模型重新润色名称/描述/内容
...
- **注释驱动生成**：识别代码中的注释需求，生成完整实现并保留方法签名

### 📝 AI生成Commit信息功能（仅在启用 Git 支持的 IDE 中可用）

- **启用条件**：需要 JetBrains Git 插件已启用，并且项目存在可用的 Git 仓库
- **智能分析**：自动分析文件变更和代码差异
...
### 🛠️ 模板调试器 (Template Debugger)
- **专为 Prompt 优化设计**: 提供独立的调试环境，用于测试和打磨 Prompt 模板
- **实时预览**: 快速查看 Prompt 在不同输入下的表现
- **便捷调试**: 无需在编辑器中反复撤销重做，直接在调试器中迭代 Prompt

### 👁️ 预览窗口 / 阅读源码模式 (Preview Window)
- **安全预览**: 在应用更改前，通过左右对比视图（Diff View）查看 AI 转换结果
- **阅读源码模式**: 像阅读源码一样审查 AI 生成的代码，确保逻辑正确性
- **无损体验**: 原始代码保持不变，直到你确认应用更改
- **配置方式**: 在设置中将 "输出模式" 切换为 "预览窗口" 即可启用
