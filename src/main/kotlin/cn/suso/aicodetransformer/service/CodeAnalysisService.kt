...
class CodeAnalysisService {
     ...
     * 检测选中代码是否为方法并分析其参数
     */
    fun analyzeSelectedCode(selectedCode: String): MethodAnalysisResult {
        try {
            val trimmedCode = selectedCode.trim()
            
            // 检测是否为方法
            val methodInfo = extractMethodInfo(trimmedCode)
            ...
            if (methodInfo == null) {
                return MethodAnalysisResult(
                    ...
                    hasResponseParameters = false,
                    errorMessage = "选中的代码不是一个有效的方法。请选择完整的方法定义，包括方法签名和方法体。"
                )
            }
            
            // 检查是否有请求参数（方法参数）
            val hasRequestParameters = methodInfo.parameters.isNotEmpty()
            
            // 检查是否有响应参数（返回值不为void）
            val hasResponseParameters = methodInfo.returnType != "void" && 
                                      methodInfo.returnType != "Unit" && 
                                      methodInfo.returnType.isNotBlank()
            
            // 根据不同情况提供详细的错误信息
            when {
                !hasRequestParameters && !hasResponseParameters -> {
                    return MethodAnalysisResult(
                        isMethod = true,
                        methodInfo = methodInfo,
                        hasRequestParameters = false,
                        hasResponseParameters = false,
                        errorMessage = "该方法没有参数且没有返回值，无法进行代码转换。请选择包含请求参数或返回值的方法。"
                    )
                }
                !hasRequestParameters -> {
                    return MethodAnalysisResult(
                        ...
                        methodInfo = methodInfo,
                        hasRequestParameters = false,
                        hasResponseParameters = true,
                        errorMessage = "该方法没有参数，无法获取请求参数信息。请选择包含参数的方法。"
                    )
                }
                !hasResponseParameters -> {
                    return MethodAnalysisResult(
                        ...
                        hasResponseParameters = false,
                        errorMessage = "该方法没有返回值，无法获取响应参数信息。请选择有返回值的方法。"
                    )
                }
            }
            ...
            return MethodAnalysisResult(
                ...
                hasResponseParameters = hasResponseParameters
            )
